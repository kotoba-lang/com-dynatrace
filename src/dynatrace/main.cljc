(ns dynatrace.main
  "Kotodama WASM entrypoint for the Dynatrace clean-room actor (L4) — Clojure port.
  ...
  facts (`dynatrace.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dynatrace")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dynatrace.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dynatrace-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dynatrace-compat/ready)
